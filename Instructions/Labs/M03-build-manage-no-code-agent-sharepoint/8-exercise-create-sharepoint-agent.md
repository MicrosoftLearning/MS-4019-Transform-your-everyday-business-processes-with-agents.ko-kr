...
이 연습에서는 빠른 답변을 얻거나 이미 사용 중인 특정 SharePoint 사이트와 관련된 유용한 작업을 수행하는 데 도움이 되는 SharePoint 에이전트를 만들려고 합니다. 다음 옵션 중 하나를 사용하여 이 연습을 수행할 수 있습니다.

- 팀 사이트, 프로젝트 사이트 또는 리소스 허브와 같은 SharePoint 사이트에 액세스할 수 있는 경우 이 연습에서 사이트를 사용할 수 있습니다.
- SharePoint 사이트에 액세스할 수 없는 경우 시뮬레이션된 랩 환경을 사용하여 이 연습의 단계를 계속 수행할 수 있습니다. 시뮬레이션은 가상의 Fabrikam용 SharePoint 사이트를 사용합니다. 

1. 이번 연습에 사용할 사이트가 있는 경우 **Microsoft 365** 홈페이지로 이동하고 탐색 창에서 **앱**을 선택한 다음, **SharePoint**를 선택합니다. 탐색 창에 표시되는 사이트를 검토합니다. 선택하는 사이트는 프로젝트 정보, 팀 리소스 또는 기타 지식 원본이 있는 문서, 목록 또는 페이지를 포함하는 사이트여야 합니다. 예시:
   - 공유 문서가 있는 개인 OneDrive 사이트.
   - Teams에 연결된 SharePoint 사이트
   - 공유 프로젝트 또는 부서 사이트<br>

    사용하려는 SharePoint 사이트를 선택합니다. 

1. 이 연습에 사용할 SharePoint 사이트가 없는 경우 다음 링크를 선택하여 브라우저에서 시뮬레이션된 랩 환경을 엽니다. [시뮬레이션된 SharePoint 에이전트 랩을 시작합니다](https://cvsr8rrncj-imkzu8x04l.app.highlights.guide/sites/Mark8ProjectTeam).

    이 시뮬레이션은 Fabrikam용 Mark 8 팀 사이트를 엽니다.
...
1. 사이트에 대한 새 에이전트를 만드는 프로세스는 다음 네 위치에서 시작할 수 있습니다.
    ...
    사용자 고유의 SharePoint 사이트를 사용하든 시뮬레이션된 사이트를 사용하든, 사이트의 홈페이지를 사용하겠습니다. 사이트 홈페이지에서 **+새로 만들기**를 선택한 다음, 나타나는 드롭다운 메뉴에서 **에이전트**를 선택합니다.
...
1. **신규 에이전트 생성** 양식에는 **개요**, **원본** 및 **동작**의 세 가지 탭이 있습니다. **개요** 탭은 기본적으로 처음에 열려 있습니다. 이 탭에서 에이전트의 이름과 용도를 입력해야 합니다. 각 필드에 대해 기본값이 제공되지만 원하는 경우 지금 변경할 수 있습니다. 
1. **개요** 탭에는 에이전트와 연결된 기본 아이콘도 표시됩니다. 시뮬레이션된 랩을 사용하는 경우 에이전트의 아이콘을 변경할 수 없습니다. 그러나 고유한 개인 사이트를 사용하며 아이콘을 사용자 지정하려는 경우, 아이콘 아래에 표시되는 **변경** 옵션을 선택할 수 있습니다. 에이전트 아이콘은 크기가 1MB를 초과하지 않는 .png 파일이어야 합니다. 사용할 아이콘이 없는 경우 다음 단계를 진행합니다.
...
1. 시뮬레이션을 사용하는 경우 이 시점에서 연습을 완료했습니다. 시뮬레이션은 에이전트를 편집하도록 프로그래밍되지 않습니다. 그러나 고유한 개인 SharePoint 사이트를 사용하며 에이전트를 변경하려는 경우 이 마지막 단계를 수행할 수 있습니다. 이를 위해선 에이전트 창의 위쪽 모서리에서 줄임표 아이콘을 선택합니다. 표시되는 드롭다운 메뉴에서 **에이전트 편집**을 선택합니다. 이렇게 하면 기본적으로 **새 에이전트 만들기** 창의 복제본인 **에이전트 편집** 창이 열립니다. 탭을 이동하면서 변경하려는 속성을 업데이트한 다음, 변경 내용을 저장합니다. 
